...
class MultiSourceSequenceGenerator(object):
    def __init__(
    ...
    def generate_batched_itr(
        ...
        for sample in data_itr:
            ...
            srclen = encoder_input['src_tokens'][0].size(1)
            ...
            for i, id in enumerate(s['id'].data):
                # remove padding
                src_1 = utils.strip_pad(input['src_tokens'][0].data[i, :], self.pad)
                src_2 = utils.strip_pad(input['src_tokens'][1].data[i, :], self.pad)
                ref = utils.strip_pad(s['target'].data[i, :], self.pad) if s['target'] is not None else None
                yield id, [src_1, src_2], ref, hypos[i]
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for model in self.models:
            ...
            new_order = new_order.to(src_tokens[0].device).long()
            encoder_out = model.encoder.reorder_encoder_out(encoder_out, new_order)
            encoder_outs.append(encoder_out)

        # initialize buffers
        scores = src_tokens[0].data.new(bsz * beam_size, maxlen + 1).float().fill_(0)
        scores_buf = scores.clone()
        tokens = src_tokens[0].data.new(bsz * beam_size, maxlen + 2).fill_(self.pad)
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if avg_attn_scores is not None:
                if attn is None:
                    attn = scores.new(bsz * beam_size, src_tokens[0].size(1), maxlen + 2)
                    attn_buf = attn.clone()
                    # HACK: src_tokens[0] may have problem here
                    nonpad_idxs = src_tokens[0].ne(self.pad)
